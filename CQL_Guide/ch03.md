...
* Literals that being with 0x are interpreted as hex
...
expressions contexts you can write this important pattern:
...
The `IN` operator behaves like a series of equality tests `==` not `IS` tests.  `NOT IN` behaves like a series of inequality tests.
