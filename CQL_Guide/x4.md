...
There is an unfortunate memory leak in older versions of SQLite (research pending on particular versions).  It causes this pattern to leak:

```
create table foo(id integer, id2 integer);
create table bar(id integer, id2 integer);

insert into foo(id, id2)
  select foo.id2, bar.id2 from
  foo inner join bar on foo.id = bar.id -- any join will do, even a silly one like this
  where foo.id = 1;
```
...
```
WITH
cte (id, id2) as (select foo.id2, bar.id2
  from foo
