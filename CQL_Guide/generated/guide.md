...
-----
### CQL0343: the variable must be of type `object<T cursor>` where T is a valid shape name 'variable'
...
### CQL0344: the cursor did not originate from a SQLite statement, it only has values 'name'

It's possible to convert a statement cursor (i.e. one that captured a SQLite select statement) into
an object using the form:

```
set X from cursor C;
```

This error indicates that the cursor `C` in this example did not come from a SQL statement.  It is most
likely a value cursor created with something like `declare C cursor like T`.  Such cursors do not
move forward over a statement so passing them around is of limited value.  Rather you can forward (some of) their values as needed.
