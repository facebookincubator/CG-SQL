...
When you include the amalgam, by default you get it all, however you might only want some
limited subset of the compiler functions in your build.  To allow this there are a set
of configuration pre-processor options.  To opt-in first define `CQL_AMALGAM_LEAN`.

Once `CQL_AMALGAM_LEAN` is set, you have to opt-in to the various pieces you might want,
the system is useless without the parser so you can't remove that.  But you can choose
from the following.  Note that `CQL_AMALGAM_SEM` is necessary for any of the code generation
