...
#define EXPLAIN_OP_NONE 1
#define EXPLAIN_OP_QUERY_PLAN 2
...
cql_noexport ast_node *_Nonnull new_asti(int32_t value);
cql_noexport ast_node *_Nonnull new_astl(int64_t value);
cql_noexport ast_node *_Nonnull new_astd(double value);
cql_noexport ast_node *_Nonnull new_asts(const char *_Nonnull value);
cql_noexport ast_node *_Nonnull new_asts_cstr(const char *_Nonnull value);
...
cql_noexport bool_t is_ast_lng(ast_node *_Nullable node);
cql_noexport bool_t is_ast_dbl(ast_node *_Nullable node);
...
#define EXTRACT_INT(name, node) \
  Contract(is_ast_int(node)); \
  int32_t name = (int32_t)((int_ast_node *)node)->value;

#define EXTRACT_LONG(name, node) \
  Contract(is_ast_lng(node)); \
  int64_t name = ((int_ast_node *)node)->value;

#define EXTRACT_DOUBLE(name, node) \
  Contract(is_ast_dbl(node)); \
  double name = ((dbl_ast_node *)node)->value;
...
AST_DATA_DECL( CSTR _Nonnull k_ast_lng );
AST_DATA_DECL( CSTR _Nonnull k_ast_dbl );
AST_DATA_DECL( CSTR _Nonnull k_ast_str );
AST_DATA_DECL( CSTR _Nonnull k_ast_blob );

AST_DATA_DEFN( CSTR _Nonnull k_ast_int = "int" );
AST_DATA_DEFN( CSTR _Nonnull k_ast_lng = "lng" );
AST_DATA_DEFN( CSTR _Nonnull k_ast_dbl = "dbl" );
