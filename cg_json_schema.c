...
static void cg_json_attr_value(charbuf *output, ast_node *ast) {
  if (is_ast_misc_attr_value_list(ast)) {
    ...
    if (is_ast_strlit(ast)) {
      ...
      cg_encode_c_string_literal(str1.ptr, &str2);
    ...
    gen_set_output_buffer(output);
    gen_with_callbacks(ast, gen_root_expr, NULL);
  }
}
...
static void cg_json_ad_hoc_migration_procs(charbuf* output) {
  ...
  for (list_item *item = all_ad_hoc_list; item; item = item->next) {
    ...
    EXTRACT_INT(version, version_annotation->left);
...
static bool_t cg_json_param(charbuf *output, ast_node *ast) {
  ...
  if (opt_inout && !is_ast_in(opt_inout)) {
    simple = 0;
  }
  ...
  if (!simple) {
    if (is_ast_inout(opt_inout)) {
      ...
      // This is the remaining legal choice
      Contract(is_ast_out(opt_inout));
      bprintf(output, ",\n\"binding\" : \"out\"", name);
    }
  }
  END_INDENT(type);
...
// * a fragment for each inserted value with its args
static void cg_json_insert_stmt(charbuf *output, ast_node *ast) {
...
bool_t static is_simple_insert(ast_node *ast) {
  ...
  return !is_select_stmt(columns_values->right);
}
...
static void cg_json_general_proc(ast_node *ast, ast_node *misc_attrs, CSTR params) {
  ...
  bool_t struct_result = !!(sem_type & SEM_TYPE_USES_OUT);
  bool_t rowset_result = !struct_result && ast->sem->sptr;
  bprintf(output, ",\n\"usesDatabase\" : %d", !!(sem_type & SEM_TYPE_DML_PROC));
  bprintf(output, ",\n\"hasStructResult\" : %d", struct_result);
  bprintf(output, ",\n\"hasRowsetResult\" : %d", rowset_result);
  if (struct_result || rowset_result) {
...
static void cg_json_create_proc(ast_node *ast, ast_node *misc_attrs) {
  ...
  CHARBUF_OPEN(tmp);
    // quote the file as a c style literaj
  ...
  else if (simple && is_simple_insert(stmt)) {
    output = inserts;
    cg_begin_proc(output, ast, misc_attrs);
    BEGIN_INDENT(proc, 2);
    bprintf(output, "%s", param_buffer.ptr);
    cg_json_insert_stmt(output, stmt);
    END_INDENT(proc);
  }
  else if (simple && is_ast_delete_stmt(stmt)) {
    output = deletes;
    cg_begin_proc(output, ast, misc_attrs);
    BEGIN_INDENT(proc, 2);
    bprintf(output, "%s", param_buffer.ptr);
  ...
  else if (simple && is_ast_update_stmt(stmt)) {
    output = updates;
    cg_begin_proc(output, ast, misc_attrs);
    BEGIN_INDENT(proc, 2);
    bprintf(output, "%s", param_buffer.ptr);
