...
cql_noexport bool_t is_ast_lng(ast_node *node) {
  return node && (node->type == k_ast_lng);
}

cql_noexport bool_t is_ast_dbl(ast_node *node) {
  return node && (node->type == k_ast_dbl);
}
...
cql_noexport bool_t is_ast_primitive(ast_node *node) {
  return is_ast_str(node) || is_ast_blob(node) || is_ast_dbl(node) || is_ast_int(node) || is_ast_lng(node);
}
...
cql_noexport ast_node *new_ast(const char *type, ast_node *left, ast_node *right) {
  ...
  ast_node *nast = _ast_pool_new(ast_node);
  nast->type = type;
  nast->left = left;
  nast->right = right;
  ...
  if (left) left->parent = nast;
  if (right) right->parent = nast;

  return nast;
}

cql_noexport ast_node *new_asti(int32_t value) {
  Contract(current_file && yylineno > 0);
...
cql_noexport ast_node *new_astl(int64_t value) {
  Contract(current_file && yylineno > 0);
  int_ast_node *iast = _ast_pool_new(int_ast_node);
  iast->type = k_ast_lng;
  iast->value = value;
...
cql_noexport ast_node *new_astd(double value) {
  Contract(current_file && yylineno > 0);
  dbl_ast_node *dast = _ast_pool_new(dbl_ast_node);
  dast->type = k_ast_dbl;
  dast->value = value;
  dast->lineno = yylineno;
  dast->filename = current_file;
  dast->sem = NULL;
  return (ast_node *)dast;
}

cql_noexport ast_node *new_asts(CSTR value) {
  Contract(current_file && yylineno > 0);
...
cql_noexport ast_node *new_asts_cstr(CSTR value) {
  value = convert_cstrlit(value);
  str_ast_node *sast = (str_ast_node *)new_asts(value);
...
cql_noexport bool_t print_ast_value(struct ast_node *node) {
  ...
  if (is_ast_lng(node)) {
    cql_output("%s", padbuffer);
    cql_output("{longint %lld}", (llint_t)((struct int_ast_node *)node)->value);
    ret = true;
  }

  if (is_ast_dbl(node)) {
    cql_output("%s", padbuffer);
    cql_output("{dbl %f}", ((struct dbl_ast_node *)node)->value);
    ret = true;
  }
...
cql_noexport ast_node *copy_ast_tree(ast_node *_Nonnull node) {
  ...
  if (is_ast_int(node)) {
    EXTRACT_INT(value, node);
    new_node = new_asti(value);
  } else if (is_ast_lng(node)) {
    EXTRACT_LONG(value, node);
    new_node = new_astl(value);
    ...
    new_node = new_asts(value);
  } else if (is_ast_dbl(node)) {
    EXTRACT_DOUBLE(value, node);
    new_node = new_astd(value);
  } else {
