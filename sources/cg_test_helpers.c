...
static void find_all_table_nodes_left_right(dummy_test_info *info, ast_node *node) {
  // Check the left and right nodes.
  if (ast_has_left(node)) {
    find_all_table_nodes(info, node->left);
  }
  if (ast_has_right(node)) {
    find_all_table_nodes(info, node->right);
  }
}
...
static void find_all_triggers_node(dummy_test_info *info, CSTR table_or_view_name) {
  ...
  if (triggers_entry &&
    ...
    for (int32_t i = 0; i < count; i++) {
      ...
      find_all_table_nodes_left_right(info, create_trigger_stmt);
    }
  }
}
...
static void find_all_table_nodes(dummy_test_info *info, ast_node *node) {
  ...
  if (table_or_view_name_ast) {
    ...
    // tables that are deleted have no business appearing in the dummy test output
  ...
  // Check the left and right nodes.
  find_all_table_nodes_left_right(info, node);
}
