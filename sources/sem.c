...
static void sem_special_func_cql_blob_create(ast_node *ast, uint32_t arg_count, bool_t *is_aggregate) {
  ...
  *is_aggregate = false;
  ...
  if (!is_ast_str(backed_name_ast)) {
    report_error(backed_name_ast, "CQL0257: argument 1 must be a table name that is a backed table", "cql_blob_create");
    record_error(ast);
    return;
  }
  ...
  for (ast_node *next_arg = arg_list->right; next_arg; next_arg = next_arg->right->right) {
    ...
    if (!is_ast_dot(table_expr) || !is_ast_str(table_expr->left) || !is_ast_str(table_expr->right)) {
      report_error(table_expr, "CQL0257: argument must be table.column where table is a backed table", "cql_blob_create");
      record_error(ast);
      return;
    }
...
static void sem_special_func_cql_blob_update(ast_node *ast, uint32_t arg_count, bool_t *is_aggregate) {
  ...
  *is_aggregate = false;
  ...
  if (!is_ast_dot(dot) || !is_ast_str(dot->left) || !is_ast_str(dot->right)) {
    report_error(dot, "CQL0257: argument must be table.column where table is a backed table", "cql_blob_update");
    record_error(ast);
    return;
  }
...
static void sem_special_func_cql_blob_get(ast_node *ast, uint32_t arg_count, bool_t *is_aggregate) {
  ...
  *is_aggregate = false;
  ...
  if (!is_ast_dot(table_expr) || !is_ast_str(table_expr->left) || !is_ast_str(table_expr->right)) {
    report_error(table_expr, "CQL0257: argument must be table.column where table is a backed table", "cql_blob_get");
    record_error(ast);
    return;
  }
