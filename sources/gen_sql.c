...
static void gen_type_kind(CSTR name) {
  // we don't alway shave an ast node for this, we make a fake one for the callback
  str_ast_node sast = { 
    .type = k_ast_str,
    .value = name,
    .filename = "none"
...
static void _gen_declare_proc_from_create_proc(ast_node *ast) {
  Contract(is_ast_create_proc_stmt(ast));
  Contract(!for_sqlite());
  EXTRACT_STRING(name, ast->left);
  EXTRACT_NOTNULL(proc_params_stmts, ast->right);
...
static bool_t gen_found_set_kind(ast_node *ast, void *context, charbuf *buffer) {
  ...
  if (proc) {
    // get canonical name
    EXTRACT_STRING(name, get_proc_name(proc));

    // we interrupt the current decl to emit a decl for
    // this new name
    if (!closure_emitted || symtab_add(closure_emitted, name, NULL)) {
...
cql_noexport void gen_declare_proc_closure(ast_node *ast, symtab *emitted) {
  ...
  CHARBUF_OPEN(current);
    gen_output = &current;
    _gen_declare_proc_from_create_proc(ast);
