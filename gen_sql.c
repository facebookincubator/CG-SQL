...
static void gen_expr_int(ast_node *ast, CSTR op, int32_t pri, int32_t pri_new) {
  Contract(is_ast_int(ast));
  EXTRACT_INT(lit, ast);
  gen_printf("%d", lit);
}

static void gen_expr_lng(ast_node *ast, CSTR op, int32_t pri, int32_t pri_new) {
  Contract(is_ast_lng(ast));
  EXTRACT_LONG(lit, ast);
  gen_printf("%lld", (long long int)lit);
}

static void gen_expr_dbl(ast_node *ast, CSTR op, int32_t pri, int32_t pri_new) {
  Contract(is_ast_dbl(ast));
  EXTRACT_DOUBLE(dbl, ast);
  gen_printf("%f", dbl);
}
...
static void gen_select_core(ast_node *ast) {
  Contract(is_ast_select_core(ast));
  EXTRACT(select_opts, ast->left);
...
static void gen_explain_stmt(ast_node *ast) {
  ...
  EXTRACT_INT(explain_op, ast->left);
  EXTRACT_ANY_NOTNULL(stmt_target, ast->right);

  gen_printf("EXPLAIN");
  if (explain_op == EXPLAIN_OP_QUERY_PLAN) {
...
cql_noexport void gen_init() {
  ...
  EXPR_INIT(int, gen_expr_int, "INT", EXPR_PRI_ROOT);
  EXPR_INIT(dbl, gen_expr_dbl, "DBL", EXPR_PRI_ROOT);
  EXPR_INIT(lng, gen_expr_lng, "LNG", EXPR_PRI_ROOT);
