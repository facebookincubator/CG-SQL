...
static void cg_dummy_test_column_value(charbuf *output, ast_node *value) {
  if (is_ast_uminus(value)) {
    Contract(is_ast_numeric(value->left));
    bprintf(output, "%s", "-");
    value = value->left;
  }
  ...
  else if (is_ast_int(value)) {
    EXTRACT_INT(lit, value);
    bprintf(output, "%d", lit);
  }
  else if (is_ast_lng(value)) {
    EXTRACT_LONG(lit, value);
    bprintf(output, "%lldL", (llint_t)lit);
  }
  else if (is_ast_dbl(value)) {
    EXTRACT_DOUBLE(lit, value)
    bprintf(output, "%f", lit);
  }
...
static bool_t is_column_value_exist(bytebuf *column_values, sem_t column_type, ast_node *value) {
  ...
  for (int32_t i = 0; i < size; i++) {
    ...
    if (is_ast_uminus(l)) {
      Contract(is_ast_numeric(l->left));
      l = l->left;
    }
    ...
    if (col_type == SEM_TYPE_LONG_INTEGER ||
      ...
      exist = ((int_ast_node *)l)->value == ((int_ast_node *)value)->value;
    }
    else if (col_type == SEM_TYPE_TEXT) {
      exist = !Strcasecmp(((str_ast_node *)l)->value, ((str_ast_node *)value)->value);
    }
    else if (col_type == SEM_TYPE_REAL) {
      exist = ((dbl_ast_node *)l)->value == ((dbl_ast_node *)value)->value;
    }
    if (exist) {
      return true;
    }
