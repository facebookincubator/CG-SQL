...
-- + DECLARE C CURSOR LIKE SELECT 1 AS A, 2.500000 AS B, 'x' AS C;
...
-- + Error % extension fragment CTE must have a FROM clause an no other top level clauses 'bogus_extra_clauses'
-- +1 Error
@attribute(cql:extension_fragment=core)
...
-- TEST: extension_fragment attribute (correct usage)
-- + {create_proc_stmt}: select: { x: integer notnull, y: text, z: longint, name: text } dml_proc
-- + {select_stmt}: select: { x: integer notnull, y: text, z: longint, name: text }
-- + {select_core_list}: union_all: { x: integer notnull, y: text, z: longint, name: text }
-- + {select_core_compound}
-- - Error
@attribute(cql:extension_fragment=core)
...
begin
  with
    core(x,y,z) as (select 1,nullable("a"),nullable(3L)), -- this is stub for the core
    test_bad_extension_fragment_union_three(x,y,z,name) as (
    select bar.*, CAST(NULL as TEXT) as name from bar
    union all
    select bar.*, plugin_table.name from bar inner join plugin_table on plugin_table.name = "test")
...
-- +1 Error
@attribute(cql:extension_fragment=core)
create proc test_extension_fragment_union_four(id_ integer not null)
begin
  with
    core(x,y,z) as (select 1,nullable("a"),nullable(3L)), -- this is stub for the core
    test_bad_extension_fragment_union_four(x,y,z,name) as (
    select core.x, core.y, CAST(NULL as INT) as z, CAST(NULL as TEXT) as name from core
    union all
    select core.x, core.y, plugin_table.id as z , plugin_table.name from core inner join plugin_table on plugin_table.name = "test")
  select * from test_bad_extension_fragment_union_four;
end;
...
begin
  with
    ...
    assembly_one(x,y,z,a,b,c) as (
    select assembly_core.*, plugin_table.flag, min(plugin_table.id, 10L), min(plugin_table.id, 10.05) from assembly_core
...
begin
  with
    ...
    assembly_two(x,y,z,name) as (
    select assembly_core.*, CAST(NULL as TEXT) as name from assembly_core
    union all
    select assembly_core.*, plugin_table.name from assembly_core inner join plugin_table on plugin_table.name = "test")
...
-- + {create_proc_stmt}: select: { x: integer notnull, y: text, z: longint, a: bool, b: longint, c: real, name: text } dml_proc
-- + {cte_table}: assembly_core: { x: integer notnull, y: text, z: longint }
-- + {cte_table}: assembly_one: { x: integer notnull, y: text, z: longint, a: bool, b: longint, c: real }
-- + {cte_table}: assembly_two: { x: integer notnull, y: text, z: longint, a: bool, b: longint, c: real, name: text }
-- + {select_stmt}: select: { x: integer notnull, y: text, z: longint, a: bool, b: longint, c: real, name: text }
...
-- + {upsert_stmt}: err
-- + {insert_stmt}: err
-- + Error % upsert statement require a where statement if the insert statement has a select statement
-- +1 Error
...
-- + foo (a, b, c) AS (SELECT 1 AS a, 'b' AS b, 3.000000 AS c)
