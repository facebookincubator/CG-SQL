...
// Integer literal, spit it out.
static void cg_expr_int(ast_node *expr, CSTR op, charbuf *is_null, charbuf *value, int32_t pri, int32_t pri_new) {
  Contract(is_ast_int(expr));
  EXTRACT_INT(lit, expr);
  // int literal
  bprintf(value, "%d", lit);
  bprintf(is_null, "0");
}

// Long integer literal, spit it out.
static void cg_expr_lng(ast_node *expr, CSTR op, charbuf *is_null, charbuf *value, int32_t pri, int32_t pri_new) {
  Contract(is_ast_lng(expr));
  EXTRACT_LONG(lit, expr);
  // long int literal
  bprintf(value, "%lldL", (llint_t)lit);
  bprintf(is_null, "0");
}

// Double literal, spit it out.
static void cg_expr_dbl(ast_node *expr, CSTR op, charbuf *is_null, charbuf *value, int32_t pri, int32_t pri_new) {
  Contract(is_ast_dbl(expr));
  EXTRACT_DOUBLE(dbl, expr);
  // double literal
  bprintf(value, "%f", dbl);
  bprintf(is_null, "0");
}
...
static void cg_c_init(void) {
  ...
  EXPR_INIT(int, cg_expr_int, "int32_t", C_EXPR_PRI_ROOT);
  EXPR_INIT(lng, cg_expr_lng, "int64_t", C_EXPR_PRI_ROOT);
  EXPR_INIT(dbl, cg_expr_dbl, "DBL", C_EXPR_PRI_ROOT);
