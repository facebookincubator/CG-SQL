...
cql_noexport bool_t is_ast_numeric(ast_node *node) {
  return is_ast_int(node) || is_ast_lng(node) || is_ast_dbl(node);
}
...
static bool_t find_referenceable_columns(
  ...
  for (int32_t i = 0; i < indices->capacity; i++) {
    ...
    if (entry.sym) {
      ...
      EXTRACT_INT(flags, flags_names_attrs->left);
...
static void sem_fk_def(ast_node *table_ast, ast_node *def, version_attrs_info *table_info) {
  ...
  EXTRACT_INT(flags, fk_target_options->right);
...
static bool_t sem_validate_version(ast_node *ast, int32_t *version, CSTR *out_proc) {
  ...
  EXTRACT_INT(vers, version_annotation->left);
...
static void sem_expr_between_or_not_between(ast_node *ast, CSTR cstr) {
  ...
  if (current_expr_context == SEM_EXPR_CONTEXT_NONE) {
    ...
    ast_node *asts = new_asts(name);
    asts->sem = new_sem(expr->sem->sem_type | SEM_TYPE_VARIABLE);
    asts->sem->name = name;
    symtab_add(scope, name, asts);

    ast_node *left_item = new_asts(name);
    ast_node *right_item = new_asts(name);
    ast_node *left_cmp;
    ast_node *right_cmp;
    ast_node *combine;
    ...
    ast_set_left(range, new_asts(name));
...
static void sem_func_ntile(ast_node *ast, uint32_t arg_count) {
  ...
  ast_node *arg = first_arg(arg_list);
  if (!is_ast_int(arg) || ((int_ast_node *)arg)->value < 1) {
...
static void sem_func_nth_value(ast_node *ast, uint32_t arg_count) {
  ...
  ast_node *arg1 = first_arg(arg_list);
  ast_node *arg2 = second_arg(arg_list);
  if (!is_ast_int(arg2) || ((int_ast_node *)arg2)->value == 0) {
...
static void sem_strftime(ast_node *ast, uint32_t arg_count, bool_t has_format) {
  Contract(is_ast_call(ast));
  EXTRACT_ANY_NOTNULL(name_ast, ast->left);
  EXTRACT_STRING(name, name_ast);
  EXTRACT_NOTNULL(call_arg_list, ast->right);
...
static void sem_join_target(ast_node *ast) {
  ...
  EXTRACT_INT(join_type, ast->left);
...
// [SELECT ... FROM ... WHERE ... GROUP BY ...]. It does not include [With ...],
// [ORDERBY ... LIMIT OFFSET ...]. Note that virtually most of the node in a
// select statement are optional.
// This function execute the semantic anlysis of the select-core component.
static void sem_select_core(ast_node *ast) {
  Contract(is_ast_select_core(ast));

  EXTRACT(select_opts, ast->left);
  ...
  sem_select_expr_list_con(select_expr_list_con);
  ast->sem = select_expr_list_con->sem;
  has_dml = 1;
}
...
static void sem_select_no_with(ast_node *ast) {
  ...
  Contract(select_expr_list->sem);
  Contract(select_expr_list->sem->sptr);
  PUSH_JOIN(list_scope, sem_join_from_sem_struct(select_expr_list->sem->sptr));
  {
    PUSH_MONITOR_SYMTAB();
...
static void sem_select_core_list(ast_node *ast) {
  ...
  ast->sem = new_sem(SEM_TYPE_STRUCT);
  ast->sem->sptr = sptr;
  EXTRACT_INT(compound_operator, select_core_compound->left);
...
static void sem_explain(ast_node *stmt) {
  ...
  EXTRACT_INT(explain_op, stmt->left);
...
static void sem_validate_previous_table(ast_node *prev_table) {
  ...
  EXTRACT_INT(prev_flags, prev_table_flags_attrs->left);
  ...
  EXTRACT_INT(flags, table_flags_attrs->left);
...
static void sem_create_trigger_stmt(ast_node *ast) {
  ...
  EXTRACT_INT(action_flags, trigger_action->left);
  flags |= action_flags;
...
static void sem_column_spec_and_values(ast_node *ast, ast_node *table_ast) {
  ...
  if (is_select_stmt(columns_values->right)) {
    select_stmt = columns_values->right;
  }
  else if (columns_values->right) {
    Contract(is_ast_insert_list(columns_values->right));
    insert_list = columns_values->right;
  }
...
static ast_node *sem_generate_full_column_list(sem_struct *sptr) {
  ...
  for (int32_t i = 0; i < sptr->count; i++) {
    ast_node *ast_col = new_asts(sptr->names[i]);
...
static void sem_upsert_stmt(ast_node *stmt) {
  ...
  if (select_count == 0 && !found_where_stmt) {
    report_error(insert_stmt, "CQL0280: upsert statement require a where statement if the insert statement has a select statement", NULL);
...
static void sem_rewrite_insert_list_from_arguments(ast_node *ast, uint32_t count) {
  ...
  if (from_name) {
    ...
    for (i = 0; i < cols && i < count; i++) {
      ...
      if (name) {
        ast_node *ast_arg = new_asts(name);
    ...
    for (; params && i < count; params = params->right, i++) {
      ...
      ast_node *ast_arg = new_asts(param->sem->name);
...
static void sem_rewrite_insert_list_from_cursor(ast_node *ast, ast_node *from_cursor, uint32_t count) {
  ...
  for (int32_t i = 0; i < count; i++, item = item->right) {
    ...
    ast_node *cname = new_asts(cursor->sem->name);
    ast_node *col = new_asts(item_name);
...
// INSERT INTO T(x,y) values(C.x, C.y) is better than
//
// Better still:  INSERT INTO T(like C) from cursor C; which is coming soon
...
static ast_node *printf_col_for_dummy(CSTR col, CSTR seed_name) {
  ast_node *ast_printf = new_asts("printf");
  CSTR fmt = dup_printf("'%s_%%d'", col);  // this turns into 'col_%d'
  ast_node *ast_string = new_asts(fmt);
  ast_node *ast_seed = new_asts(seed_name);
...
static void sem_synthesize_dummy_value(dummy_info *info) {
  ...
  if (!symtab_find(scope, seed_name)) {
    ast_node *asts = new_asts(seed_name);
  ...
  ast_node *ast_col = new_asts(info->name);
  PUSH_JOIN_BLOCK()
...
static bool_t sem_rewrite_one_def(ast_node *head) {
  ...
  for (int32_t i = 0; i < count; i++) {
    ...
    ast_node *name_ast = new_asts(col_name);
...
static void sem_rewrite_one_param(ast_node *param, symtab *param_names) {
  ...
  for (int32_t i = 0; i < count; i++) {
    ...
    ast_node *name_ast = new_asts(param_name);
...
//
static ast_node *sem_find_likeable_ast(ast_node *like_ast) {
  Contract(is_ast_like(like_ast) || is_ast_from_arguments(like_ast));
      ...
      for (ast_node *column_values_list = column_name_list->right; column_values_list; column_values_list = column_values_list->right) {
        ...
        for (ast_node *list = column_values_list->left; list; list = list->right) {
          ...
          if (is_ast_uminus(misc_attr_value)) {
            Contract(is_ast_numeric(misc_attr_value->left));
            misc_attr_value = misc_attr_value->left;
          }

          bool_t ok = false;

          if (is_ast_int(misc_attr_value)) {
             // an integer literal is good for any numeric type
             ok = is_numeric(core_type);
          }
          else if (is_ast_lng(misc_attr_value)) {
             // an long literal is only good for a long column
             ok = core_type == SEM_TYPE_LONG_INTEGER;
          }
          else if (is_ast_dbl(misc_attr_value)) {
...
static bool_t sem_extension_left_outer_join(CSTR base_frag_name, ast_node *my_select_expr_list_con, ast_node *my_cte_tables)
{
  ...
  EXTRACT_INT(join_type, join_target->left);
  ...
  if (my_any_select_core_list->right == NULL) {
    ...
    if (sem_has_extra_clauses(select_from_etc, select_orderby)) {
      report_error(select_from_etc,
         "CQL0320: extension fragment CTE must have a FROM clause an no other top level clauses", my_cte_name);
      goto error;
    }
  }
  else {
    // TODO: T68718147
    // no additional checking of the compound case at this time (there is some missing per chapter 14 of the programming guide)
    // however I'm not adding any at this time in this diff anyway [rmariani]
  }
...
static void sem_rewrite_one_typed_name(ast_node *like, symtab *used_names) {
  ...
  for (int32_t i = 0; i < count; i++) {
    ...
    ast_node *type = sem_generate_data_type(sem_type);
    ast_node *name_ast = new_asts(name);
...
static void sem_declare_proc_stmt(ast_node *ast) {
  ...
  EXTRACT_STRING(name, name_ast);
  EXTRACT_INT(type, proc_name_type->right);
  EXTRACT_NOTNULL(proc_params_stmts, ast->right);
  EXTRACT(params, proc_params_stmts->left);
...
// Expression type for integer primitives
static void sem_expr_int(ast_node *ast, CSTR cstr) {
...
// Expression type for long integer primitives
static void sem_expr_lng(ast_node *ast, CSTR cstr) {
  Contract(is_ast_lng(ast));
  // long literal
  ast->sem = new_sem(SEM_TYPE_LONG_INTEGER | SEM_TYPE_NOTNULL);
}

// Expression type for double primitives
static void sem_expr_dbl(ast_node *ast, CSTR cstr) {
  Contract(is_ast_dbl(ast));
  // double literal
...
static void sem_walk_regions(region_walk *acc, CSTR name) {
  ...
  for (ast_node *item = region_list; item; item = item->right) {
    ...
    EXTRACT_NOTNULL(region_spec, item->left);
    EXTRACT_STRING(item_name, region_spec->left);
    EXTRACT_INT(type, region_spec->right);
...
static void sem_validate_previous_ad_hoc(ast_node *prev, CSTR name, int32_t version) {
  ...
  EXTRACT_INT(vers, vers_annotation->left);
...
cql_noexport void sem_main(ast_node *ast) {
  ...
  EXPR_INIT(int, sem_expr_int, "INT");
  EXPR_INIT(lng, sem_expr_lng, "LNG");
  EXPR_INIT(dbl, sem_expr_dbl, "DBL");
