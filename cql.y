...
/* ddl stuff */
%type <ival> opt_temp opt_if_not_exists opt_unique opt_no_rowid dummy_modifier compound_operator explain_op
...
%type <aval> select_expr select_expr_list select_opts select_stmt select_core explain_stmt explain_target
...
%type <aval> declare_schema_region_stmt declare_deployable_region_stmt call
...
%type <aval> expr basic_expr math_expr expr_list typed_name typed_names case_list
...
explain_stmt: EXPLAIN explain_op explain_target { $$ = new_ast_explain_stmt(new_asti($2), $3); }
  ;

explain_op: /* nil */ { $$ = EXPLAIN_OP_NONE; }
  | QUERY_PLAN { $$ = EXPLAIN_OP_QUERY_PLAN; }
  ;
...
schema_upgrade_version_stmt: AT_SCHEMA_UPGRADE_VERSION '(' INTLIT ')' {
    $$ = new_ast_schema_upgrade_version_stmt(new_asti($3)); }
  ;
...
drop_table_stmt: DROP TABLE IF EXISTS name { $$ = new_ast_drop_table_stmt(new_asti(1), $5);  }
  | DROP TABLE name { $$ = new_ast_drop_table_stmt(NULL, $3);  }
  ;

drop_view_stmt: DROP VIEW IF EXISTS name { $$ = new_ast_drop_view_stmt(new_asti(1), $5);  }
  | DROP VIEW name { $$ = new_ast_drop_view_stmt(NULL, $3);  }
  ;

drop_index_stmt: DROP INDEX IF EXISTS name { $$ = new_ast_drop_index_stmt(new_asti(1), $5);  }
  | DROP INDEX name { $$ = new_ast_drop_index_stmt(NULL, $3);  }
  ;

drop_trigger_stmt: DROP TRIGGER IF EXISTS name { $$ = new_ast_drop_trigger_stmt(new_asti(1), $5);  }
...
create_table_stmt: CREATE opt_temp TABLE opt_if_not_exists name '(' col_key_list ')' opt_no_rowid version_attrs_opt_recreate {
    int flags = $2 | $4 | $9;
    struct ast_node *flags_node = new_asti(flags);
...
fk_target_options : REFERENCES name '(' name_list ')' opt_fk_options {
    $$ = new_ast_fk_target_options(new_ast_fk_target($2, $4), new_asti($6)); }
  ;
...
create_index_stmt: CREATE opt_unique INDEX opt_if_not_exists name ON name '(' indexed_columns ')' opt_delete_version_attr {
    ...
    ast_node *flags_names_attrs = new_ast_flags_names_attrs(new_asti(flags), index_names_and_attrs);
...
name: ID { $$ = new_asts($1); }
  | TEXT { $$ = new_asts("text"); }
  | TRIGGER { $$ = new_asts("trigger"); }
  | ROWID { $$ = new_asts("rowid"); }
  ;
...
version_annotation: '(' INTLIT ',' name ')' {
    $$ = new_ast_version_annotation(new_asti($2), $4); }
  | '(' INTLIT ')' {
    $$ = new_ast_version_annotation(new_asti($2), NULL); }
  ;
...
str_literal: STRLIT { $$ = new_asts($1);}
  | CSTRLIT { $$ = new_asts_cstr($1); }
  ;

num_literal:  INTLIT { $$ = new_asti($1); }
  | LONGLIT { $$ = new_astl($1); }
  | REALLIT { $$ = new_astd($1); }
  ;
...
raise_expr:
    RAISE '(' IGNORE ')'  { $$ = new_ast_raise(new_asti(RAISE_IGNORE), NULL); }
  | RAISE '(' ROLLBACK ','  expr ')' { $$ = new_ast_raise(new_asti(RAISE_ROLLBACK), $5); }
  | RAISE '(' ABORT ','  expr ')' { $$ = new_ast_raise(new_asti(RAISE_ABORT), $5); }
  | RAISE '(' FAIL ','  expr ')' { $$ = new_ast_raise(new_asti(RAISE_FAIL), $5); }
  ;
...
select_core_compound: compound_operator select_core_list {
    $$ = new_ast_select_core_compound(new_asti($1), $2);
  }
  ;
...
opt_frame_spec: /* nil */ { $$ = NULL; }
  | frame_type frame_boundary_opts frame_exclude {
    ...
    $$ = new_ast_opt_frame_spec(new_asti(flags), expr_list);
  }
  ;
...
frame_boundary_opts: frame_boundary {
  ...
  | BETWEEN frame_boundary_start AND frame_boundary_end {
    ...
    $$ = new_ast_frame_boundary_opts(new_asti(flags), expr_list);
  }
  ;

frame_boundary_start: UNBOUNDED PRECEDING { $$ = new_ast_frame_boundary_start(new_asti(FRAME_BOUNDARY_START_UNBOUNDED), NULL); }
  | expr PRECEDING { $$ = new_ast_frame_boundary_start(new_asti(FRAME_BOUNDARY_START_PRECEDING), $1); }
  | CURRENT_ROW { $$ = new_ast_frame_boundary_start(new_asti(FRAME_BOUNDARY_START_CURRENT_ROW), NULL); }
  | expr FOLLOWING { $$ = new_ast_frame_boundary_start(new_asti(FRAME_BOUNDARY_START_CURRENT_ROW), $1); }
  ;

frame_boundary_end: expr PRECEDING { $$ = new_ast_frame_boundary_end($1, new_asti(FRAME_BOUNDARY_END_PRECEDING)); }
  | CURRENT_ROW { $$ = new_ast_frame_boundary_end(new_asti(FRAME_BOUNDARY_END_CURRENT_ROW), NULL); }
  | expr FOLLOWING { $$ = new_ast_frame_boundary_end(new_asti(FRAME_BOUNDARY_END_FOLLOWING), $1); }
  | UNBOUNDED FOLLOWING { $$ = new_ast_frame_boundary_end(new_asti(FRAME_BOUNDARY_END_UNBOUNDED), NULL); }
  ;

frame_boundary: UNBOUNDED PRECEDING { $$ = new_ast_frame_boundary(new_asti(FRAME_BOUNDARY_UNBOUNDED), NULL); }
  | expr PRECEDING { $$ = new_ast_frame_boundary(new_asti(FRAME_BOUNDARY_PRECEDING), $1); }
  | CURRENT_ROW { $$ = new_ast_frame_boundary(new_asti(FRAME_BOUNDARY_CURRENT_ROW), NULL); }
  ;
...
region_spec:
    name  { $$ = new_ast_region_spec($1, new_asti(PUBLIC_REGION)); }
  | name PRIVATE { $$ = new_ast_region_spec($1, new_asti(PRIVATE_REGION)); }
  ;
...
join_target: opt_inner_cross JOIN table_or_subquery opt_join_cond {
      ...
      struct ast_node *asti_join_type = new_asti(join_type);
...
create_view_stmt: CREATE opt_temp VIEW opt_if_not_exists name AS select_stmt opt_delete_version_attr {
  struct ast_node *flags = new_asti($2 | $4);
...
opt_insert_dummy_spec : /*nil*/  { $$ = NULL; }
  | AT_DUMMY_SEED '(' expr ')' dummy_modifier {
    $$ = new_ast_insert_dummy_spec($3, new_asti($5)); }
  ;
...
from_cursor:  FROM CURSOR name opt_column_spec {
  $$ = new_ast_from_cursor($4, $3);
}

from_arguments: FROM ARGUMENTS { $$ = new_ast_from_arguments(NULL); }
  | FROM ARGUMENTS LIKE name { $$ = new_ast_from_arguments($4); }

insert_stmt: insert_stmt_type name opt_column_spec VALUES '(' insert_list ')' opt_insert_dummy_spec {
    struct ast_node *columns_values = new_ast_columns_values($3, $6);
  ...
  | insert_stmt_type name opt_column_spec select_stmt {
...
declare_proc_stmt: DECLARE procedure name '(' params ')' {
      ast_node *proc_name_flags = new_ast_proc_name_type($3, new_asti(PROC_FLAG_BASIC));
      $$ = new_ast_declare_proc_stmt(proc_name_flags, new_ast_proc_params_stmts($5, NULL)); }
  | DECLARE procedure name '(' params ')' '(' typed_names ')' {
      ast_node *proc_name_flags = new_ast_proc_name_type($3, new_asti(PROC_FLAG_STRUCT_TYPE | PROC_FLAG_USES_DML));
      $$ = new_ast_declare_proc_stmt(proc_name_flags, new_ast_proc_params_stmts($5, $8)); }
  | DECLARE procedure name '(' params ')' USING TRANSACTION {
      ast_node *proc_name_flags = new_ast_proc_name_type($3, new_asti(PROC_FLAG_USES_DML));
      $$ = new_ast_declare_proc_stmt(proc_name_flags, new_ast_proc_params_stmts($5, NULL)); }
  | DECLARE procedure name '(' params ')' OUT '(' typed_names ')' {
      ast_node *proc_name_flags = new_ast_proc_name_type($3, new_asti(PROC_FLAG_STRUCT_TYPE | PROC_FLAG_USES_OUT));
  ...
  | DECLARE procedure name '(' params ')' OUT '(' typed_names ')' USING TRANSACTION {
      ast_node *proc_name_flags = new_ast_proc_name_type($3, new_asti(PROC_FLAG_STRUCT_TYPE | PROC_FLAG_USES_OUT | PROC_FLAG_USES_DML));
      $$ = new_ast_declare_proc_stmt(proc_name_flags, new_ast_proc_params_stmts($5, $9)); }
  | DECLARE procedure name '(' params ')' OUT UNION '(' typed_names ')' {
      ast_node *proc_name_flags = new_ast_proc_name_type($3, new_asti(PROC_FLAG_STRUCT_TYPE | PROC_FLAG_USES_OUT_UNION));
  ...
  | DECLARE procedure name '(' params ')' OUT UNION '(' typed_names ')' USING TRANSACTION {
      ast_node *proc_name_flags = new_ast_proc_name_type($3, new_asti(PROC_FLAG_STRUCT_TYPE | PROC_FLAG_USES_OUT_UNION | PROC_FLAG_USES_DML));
      $$ = new_ast_declare_proc_stmt(proc_name_flags, new_ast_proc_params_stmts($5, $10)); }
  ;
...
call_stmt: CALL name '(' ')'{ $$ = new_ast_call_stmt($2, NULL); }
  | CALL name '(' expr_list ')' { $$ = new_ast_call_stmt($2, $4); }
  ;
...
trigger_operation:
    DELETE  { $$ = new_ast_trigger_operation(new_asti(TRIGGER_DELETE), NULL); }
  | INSERT  { $$ = new_ast_trigger_operation(new_asti(TRIGGER_INSERT), NULL); }
  | UPDATE opt_of { $$ = new_ast_trigger_operation(new_asti(TRIGGER_UPDATE), $2); }
  ;
...
enforcement_options:
    FOREIGN KEY ON UPDATE { $$ = new_asti(ENFORCE_FK_ON_UPDATE); }
  | FOREIGN KEY ON DELETE { $$ = new_asti(ENFORCE_FK_ON_DELETE); }
  | JOIN { $$ = new_asti(ENFORCE_STRICT_JOIN); }
  | UPSERT STATEMENT { $$ = new_asti(ENFORCE_UPSERT_STMT); }
  | WINDOW function { $$ = new_asti(ENFORCE_WINDOW_FUNC); }
  | procedure { $$ = new_asti(ENFORCE_PROCEDURE); }
  ;
...
void yyerror(char *s, ...) {
  ...
  va_list args;
  va_start(args, s);
  cql_error("Syntax error at: %s:%d", current_file, yylineno);
  vprintf(s, args);
}
...
static void print_dot(struct ast_node *node) {
  ...
  if (is_ast_int(node) || is_ast_lng(node)) {
    cql_output("\n    %s%lx [label = \"%lld\" shape=plaintext]", node->type, id, (llint_t)((struct int_ast_node*)node)->value);
  } else if (is_ast_str(node)) {
    cql_output("\n    %s%lx [label = \"%s\" shape=plaintext]", node->type, id, ((struct str_ast_node*)node)->value);
  } else if (is_ast_dbl(node)) {
    cql_output("\n    %s%lx [label = \"%f\" shape=plaintext]", node->type, id, ((struct dbl_ast_node*)node)->value);
  } else {
...
static ast_node *file_literal(ast_node *ast) {
  ...
  ast_node *ret = new_asts(Strdup(literal.ptr));
